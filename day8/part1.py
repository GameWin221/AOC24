...
for (frequency, antennas) in allAntennas.items():
    for i in range(0, len(antennas)):
        for j in range(i+1, len(antennas)):
            ...
            (anti_x1, anti_y1) = (ax + dx*2, ay + dy*2)
            (anti_x2, anti_y2) = (ax - dx, ay - dy)
            
            if anti_x1 >= 0 and anti_x1 < width and anti_y1 >= 0 and anti_y1 < height:
                antinodes.add((anti_x1, anti_y1))
            if anti_x2 >= 0 and anti_x2 < width and anti_y2 >= 0 and anti_y2 < height:
